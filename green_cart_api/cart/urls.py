...
urlpatterns = [
    ...
    path('<uuid:pk>/add-item/', AddItemView.as_view(), name='add-item'),
    path('<uuid:pk>/remove-item/', RemoveItemView.as_view(), name='remove-item'),
    path('<uuid:pk>/update-quantity/', UpdateQuantityView.as_view(), name='update-quantity'),
    path('<uuid:pk>/clear/', ClearCartView.as_view(), name='clear-cart'),
